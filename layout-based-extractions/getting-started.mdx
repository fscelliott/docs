...
* [**Integrate**](/docs/integrate) using Sensible's API, quick-extract UI, or other tools
* [**Quality control**](/docs/validate-extractions) extracted data
* [**Monitor**](/docs/metrics) extracted data in production
...
![Click to enlarge](/images/quickstart_instruct_1.png)
...
<Steps>
  <Step title="To extract a single data point from the document, click Query">
   ![Click to enlarge](/images/quickstart_instruct_2.png)
  </Step>
  <Step title="Edit the query as shown in the following screenshot by entering `checking account number (not savings)` in the query field. ">
    Give the query an ID, `account_num_checking`, then click the **Send** icon:

    ![Click to enlarge](/images/quickstart_instruct_3.png)
...
* The time period for each account. **Hint:** Use the [List method](/docs/list). For example, in this config, the `accounts_list` uses the List method.

Publish the config to save your changes.

### (Optional) Extract from your own documents

To extract data from your documents, first check if they're on Sensible's list of out-of-the-box [supported document types](/docs/library-quickstart). If not, configure your custom extractions by using the interactive [tutorial](https://app.sensible.so/tutorial/) or taking the following steps:
...
* See [prompt authoring tips](/docs/instruct)
...
* For advanced extraction strategies, see [Choosing an extraction approach](/docs/author)

#### Integrate

Get extracted document data out of Sensible and put it to work in Excel files, databases, and other destinations. See [Integrating](/docs/integrate).